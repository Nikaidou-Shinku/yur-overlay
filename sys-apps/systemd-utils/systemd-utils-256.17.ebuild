...
IUSE="+acl boot +kmod kernel-install selinux split-usr static-libs sysusers +tmpfiles test +udev ukify"
REQUIRED_USE="
...
multilib_src_configure() {
	...
	if use static-libs; then
		emesonargs+=( -Dstatic-libsystemd=true )
		if use udev; then
			emesonargs+=( -Dstatic-libudev=true )
		fi
	fi
